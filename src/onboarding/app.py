...
from .comitup_adapter import ComitupAdapter, ComitupError
...
CONFIG_PATH = "/etc/messagebox-onboarding/config.json"
STATE_PATH = "/var/lib/messagebox-onboarding/state.json"
BODY_LIMIT = 8192
HANDOFF_DELAY = 1.0
...
<body>
  <main class="shell">
    <section class="card handoff" aria-labelledby="handoff-title">
      ...
      <h1 id="handoff-title">Connecting your Message Box</h1>
      <p class="lede">This setup network will disappear while the box joins your home Wi-Fi.</p>
      <ol class="steps">
        <li>If needed, reconnect this phone to your home Wi-Fi.</li>
        <li>Keep this page open. It will retry the printed <strong>message-box address</strong>.</li>
        <li>If the setup hotspot returns, reopen it and check the Wi-Fi details.</li>
      </ol>
      <p class="status" role="status">Waiting for the box on your home network...</p>
      <a class="button secondary" href="/">Try the Message Box address now</a>
    </section>
  </main>
</body>
</html>
...
def create_app(
    ...
    *,
    ...
    def handoff(start_response, callback=None, body=_HANDOFF_HTML):
        headers = [
    ...
    def application(environ, start_response):
        ...
        try:
            if method == "GET" and path in {
                ...
                if selected_mode == "HOTSPOT":
                    return Response(
                        b"",
                        "302 Found",
                        [("Location", canonical_url), ("Content-Type", "text/plain; charset=utf-8")],
                    )(start_response)
            ...
            canonical = _canonical_request_host(environ, canonical_host)
            if not canonical:
                if method == "GET":
                    location = canonical_url.rstrip("/") + (path if path.startswith("/") else "/")
