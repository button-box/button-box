...
- **NFC reader:** The original NFC build uses the [HiLetgo PN532 NFC/RFID module kit](https://www.amazon.com/dp/B01I1J17LC) and needs soldering. If you do not want to solder the NFC connection, use a [Waveshare PN532 NFC HAT](https://www.amazon.com/dp/B0CKXZ4WJK) instead. The HAT seats directly on a Pi 4's 40-pin header and matches the current NFC software adapter. This does not remove the separate button and lamp wiring. The [European EL001 enclosure](hardware/enclosure/eu-el001/README.md) has been digitally checked with the HAT, but the S8 print and NFC range still need physical verification; do not assume the earlier prototype enclosure fits it.
