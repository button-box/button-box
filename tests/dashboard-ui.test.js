...
test("Home never treats a linked account alone as completed first-message setup", () => {
  const h = harness();
  h.run('renderHome({mode:"RUNTIME",setup:{wifi:"complete",whatsapp:"complete",recipient:"complete",first_message:"attention"}})');
  expect(h.node("home-attention").hidden).toBe(false);
  expect(h.node("home-runtime").textContent).toBe("Needs attention");
  h.run('renderHome({mode:"RUNTIME",setup:{wifi:"complete",whatsapp:"complete",recipient:"complete",first_message:"complete"}})');
