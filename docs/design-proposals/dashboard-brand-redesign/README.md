...
4. **Support identity placement:** keep the customer-facing **Box ID** out of the prominent header. Put it in a low-key footer support area with a Copy action and guidance to share it with Dan when asking for help. `BOX-4` is sample prototype data; durable persistence and recovery after reimage are not implemented.
...
- **Box ID is customer-facing proposal language, not an implemented feature.** The backend must later define one durable canonical source installed during provisioning, retained across onboarding resets, reconciled after reimage, and kept distinct from hostname and enclosure/part serials. The sample `BOX-4` value proves only the proposed footer placement and copy interaction.
